...
Dada la expansión de los drones en distintas tareas de la actualidad y del uso de las FPGAs como núcleos de procesamiento, resulta natural mezclar dichos dispositivos para beneficio mutuo. A lo largo de este proyecto se han diseñado distintos elementos software y hardware para que un programa en Python ejecutado controle por completo las operaciones y trayectoria de vuelo de un dron comercial. Este es capaz de obedecer las instrucciones de posición indicadas por el programa ejecutado sin intervención humana de por medio.\\
Para ello se ha diseñado una estructura basada en: (a) un dron equipado con un conjunto de sensores tridimensionales de posición y una radio adicional para comunicar las medidas a tierra; (b) una estación de tierra basada en FPGA que ejecuta y comunica el control de vuelo con el dron, mediante un transceptor radio; y (c) un ordenador para ejecutar la aplicación en Python y conectado por USB a la estación de tierra. Estos elementos dotan al sistema de control autónomo y han sido diseñado en base a FPGAs y drones de bajo coste, junto con electrónicas concretas para cubrir cada tarea. Finalmente se escogió un dron X5C de Syma como plataforma de vuelo mejorada mediante sensores de BitCraze, junto a una FPGA de la familia ICE40 de Lattice para la estación y procesadores periféricos de Atmel, consiguiendo así un sistema final de bajo coste.\\
Tras el diseño de los distintos elementos se han realizado ensayos de verificaciones de cada uno por separado e integraciones de los mismos. La solución completa diseñada y programada se ha validado experimentalmente con éxito. Se han conseguido resultados especialmente satisfactorios con los sistemas en bucle cerrado, tanto en la estabilización como en el control del dron escogido.
...
%\label{subsec:Estabilizar dron mediante FPGA en tierra}
	...
	\item Se diseñará una la estación de tierra que establezca la comunicación con el dron. Esta FPGA recibirá
...
\begin{itemize}
  ...
  \item Cada grado de libertad se controlará independientemente y permitirán el control en tiempo real del vehículo.
  ...
  \item Las electrónicas, FPGA y dron empleados deberán orientarse a elementos de bajo coste.
\end{itemize}
...
En el ordenador se va a ejecutar la aplicación robótica (en Python) que gobierna el comportamiento del dron. Al usuario se le ofrece una biblioteca en Python que permite el control de los movimientos del dron. La implementación de esa biblioteca se comunica vía USB con la estación de tierra. Adicionalmente la biblioteca incluye también funciones para la configuración de los algoritmos de control PID que materializan el control de bajo nivel. Las tramas transmitidas por USB incluirán información de dos elementos:
\begin{itemize}
...
\texttt{setcircle}     & Dirige una trayectoria progresiva en forma de arco entre la posición actual y el angulo indicado.    \\ \hline
...
\begin{itemize}
  \item Los parámetros: grados\_ini, grados\_fin y radio definen el arco a trazar deseado. Dicho arco se trazará partiendo de la posición actual y terminando una vez recorrido el ángulo formado por la diferencia entre grados\_fin y grados\_ini para el radio indicado.
  
  \item El parámetro clockwise sirve para trazar el arco a derechas o a izquierdas.
  
  \item duracion\_grado permite controlar la velocidad angular a la que se trazará el arco, teniendo una relación inversamente proporcional.
\end{itemize}
...
\begin{itemize}
  \item La altura de vuelo deseada se indica a través del parámetro height en centímetros.
  
  \item El parámetro duration controla la velocidad de ascenso del dron hasta la altura indicada previamente.
\end{itemize}
...
Para completar la librería la función \texttt{landing} se encarga del aterrizaje del dron hasta el contacto con el suelo, controlando la velocidad de descenso. Para ello hace uso de un solo parámetro:
...
\begin{itemize}
  \item El parámetro duration controla el tiempo de descenso del dron hasta el contacto con el suelo.
\end{itemize}
...
Este apartado describen los experimentos y pruebas realizadas durante el trabajo. Estos permitieron realizar la selección de los componentes finales a utilizar, corrección de errores y configuración de parámetros.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=16cm]{Imagenes/Secuencia_7_etq.png}
	\caption{Secuencia de vuelo con dron E010: (a) Comando, (b) Despegue, (c)-(f) ida y vuelta y (g) aterrizaje.}
	\label{fig:E010_test}
\end{figure}
\begin{figure}[h]
	\centering
	\includegraphics[width=16cm]{Imagenes/Secuencia_7_flechas_negro.png}
	\caption{Secuencia de vuelo con dron E010: (a) Comando, (b) Despegue, (c)-(f) ida y vuelta y (g) aterrizaje.}
	\label{fig:E010_test}
\end{figure}
...
\section{Pruebas básicas de vuelo del dron Syma X5C}
\label{sec:Syma X5C en Bucle abierto}

Tras las pruebas con el E010, se comenzó por conseguir el mismo nivel de control sobre el X5C original. Para ellos se modificó el código de enlace con el dron, ejecutado en el módulo 4 de la estación de tierra (Transmisor radio de órdenes de movimiento del apartado~\ref{subsubsec:GS_Mod4}), para conseguir control básico sobre el X5C.\\
...
%\begin{figure}[H]
%	\centering
%	\includegraphics[width=5cm]{Imagenes/Syma_pre.png}
%	\caption{Syma X5C en su estado original}
%	\label{fig:X5C_pre}
%\end{figure}
%\begin{figure}[H]
%	\centering
%	\includegraphics[width=5cm]{Imagenes/OnBoard_System_Original.jpg}
%	\caption{Syma X5C tras reducción de peso e instalación de electrónica adicional}
%	\label{fig:X5C_post}
%\end{figure}
...
En este punto el trabajo se centró en mejorar los parámetros PID de los controladores ubicados en la FPGA de la estación de tierra, ya que fueron diseñados para el dron original, previo al cambio de peso.\\
Además, se trabajó rotando dos baterías y controlando rigurosamente su nivel de carga. Ya que, aunque los bucles de control PID corrigen parte del error debido a la descarga, llegados a un nivel excesivamente bajo de batería, los parámetros de control mismos dejaban de ser igualmente efectivos. Entonces, la velocidad de respuesta sobre el error de posición también disminuía, lo que al final se traducía en el mismo efecto, parámetros PID inadecuados. Procurando trabajar con baterías en un rango de carga concreto se minimizó lo suficiente este efecto, lo suficiente para garantizar repetitividad entre vuelos.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=10cm, keepaspectratio]{Imagenes/Duno.png}
	\caption{Indicación luminosa de enlace completado satisfactoriamente}
...
\section{Pruebas con los bucles de control}
...
Además, se ha elegido una FPGA barata, de capacidad y velocidad modestas comparadas con otras FPGAs de gama alta. El motivo de elegir este modelo ICE40UP5K es que soporta la toolchain libre IceStudio y aunque en este TFG la hemos usado con entornos de desarrollo propietarios, esta elección permite continuar este trabajo con desarrollos posteriores ya utilizando entornos de desarrollo libres.
...
Tras las pruebas unitarias sobre los drones Eachine E010, Syma X5C y módulos software se procede a un ensayo completo de vuelo. Para ello se hace uso del sistema final, compuesto del ordenador ejecutando las instrucciones de mando en Python, la estación de tierra con la FPGA programada para ejecutar el control de vuelo y el dron X5C con la electrónica de a bordo transmitiendo su posición. Se lleva a cabo un ejercicio que incluye desde la ejecución en PC del software de mando, hasta el vuelo y aterrizaje del dron controlados desde tierra. Se trata de un viaje de ida y vuelta \cite{Dron_idaVuelta} mostrado en la secuencia de imágenes de la Figura~\ref{fig:vuelo_completo}. Adicionalmente se realizaron ensayos de despegue y aterrizaje controlado \cite{Dron_Aterrizaje} y comprobación de la estabilidad en vuelo estacionario \cite{Dron_estacionario}.

\begin{figure}[h]
	\centering
	\includegraphics[width=16cm, keepaspectratio]{Imagenes/X5C_Secuencia_7_Small.png}
	\caption{Secuencia de vuelo autónomo con dron X5C comandado desde PC: (a) Comando, (b) Despegue, (c)-(f) ida y vuelta y (g) aterrizaje.}
	\label{fig:vuelo_completo}
\end{figure}

Todo el software generado durante el desarrollo de este trabajo se encuentra disponible en el repositorio de Github asociado al TFG \cite{EloyRepo}.
...
% por este motivo se procedió a evolucionar todo el sistema, hardware y software, tanto en tierra como a bordo, dotándolo de una nueva dimensión, conocimiento en el dron de la propia ubicación relativa del dron. Esto supuso poder avanzar al siguiente tipo de control.\\
...
El trabajo realizado se pone a disposición pública \cite{EloyRepo}, tanto el hardware, software como el propio diseño del sistema, listos para que cualquiera persona pueda adquirir los elementos por un bajo coste, instalarlos y ponerlos en funcionamiento. Esto abre un gran abanico de posibles modificaciones, mejoras y desarrollos futuros.
