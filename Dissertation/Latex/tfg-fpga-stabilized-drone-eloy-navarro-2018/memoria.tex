...
Tarjeta de desarrollo para la familia UltraPlus de los modelos ICE40 de Lattice. Incluye elementos varios de interfaz humana; como Leds, interruptores, jumpers y conexiones de propósito general. También incluye un interfaz USB reprogramable de FTDI Chip y doble canal, FT232HL. Como procesador dispone de una FPGA ICE40UP5K-SG48, con 5280 celdas lógicas, 120Kbits de RAM embebida y 1024kbits de RAM estática, una PLL y 8 bloques DSP (multiplicadores de 16 bits). Esta plataforma se ha usado como núcleo de la estación de tierra, conectándose con periféricos radio y el PC de mando.
...
El vehículo se equipa con los siguientes módulos, utilizados para conocer su propia ubicación en tiempo real.
\begin{itemize}
...
Comunica por puerto serie SPI, el sensor de flujo, PMW3901, del módulo 2 con el procesador del módulo 1. Porta las medidas de velocidad de desplazamiento sobre el plano horizontal en ambos ejes, derivadas del desplazamiento del suelo bajo el sensor.
...
Este módulo, hace posible que el vehículo escogido, obedezca las instrucciones ejecutadas en el ordenador de mando (Apartado~\ref{sec: Ordenador de mando}). Para ello, hace uso de una FPGA de Lattice ubicada en la tarjeta de desarrollo escogida (\ref{sec:ICE40 UltraPlus Breakout Board}) y dos módulos radio (\ref{sec:NRF24L01}). Este conjunto de electrónica otorga las siguientes capacidades a la estación de tierra:
...
El conjunto de elementos escogidos, convergen en la siguiente arquitectura.

\subsection{Arquitectura de la estación}
\label{subsec:OB_Desc}

La estación de tierra se estructura de manera que sea capaz de realizar sus funciones, ofreciendo además facilidades para la depuración. Una vez construido el sistema completo, resulta fundamental poder realizar cómodamente tareas de ensayo y error para el ajuste de los distintos parámetros que intervienen en el control del vuelo del vehículo. Debido a esta premisa, la estación de tierra se compone de elementos que le permiten comunicarse con el dron y el ordenador de mando mediante módulos radio y USB respectivamente, procesar información mediante la FPGA, realizar capturar de valores internos y enviarlos hacia el PC para su análisis. En la Figura~\ref{fig:ETArq} se exponen los módulos e interfaces de la estación de tierra, que hacen posible dichas tareas.

\begin{figure}[h]
	\centering
	\includegraphics[width=13cm, keepaspectratio]{Imagenes/ET_arq.png}
	\caption{Arquitectura de la estación de tierra}
	\label{fig:ETArq}
\end{figure}

A continuación se explican los módulos hacen posible que la estación lleve a cabo su cometido.
...
De manera funcional, este módulo hace uso únicamente de un interfaz con el sistema. El comentado \textit{Interfaz USB de Mando}, dirigido contra la estación de tierra y mostrado en la Figura~\ref{fig:NewSystArq}.\\
Paralelo a este, existe un segundo interfaz USB, desde la estación de tierra hacia el PC, utilizado exclusivamente para tareas de depuración (no presente en~\ref{fig:NewSystArq}). Este segundo USB, se comunica con la herramienta de depuración \textit{Logic}, comentada en \ref{sec:Logic Analizer}.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=14cm, keepaspectratio]{Imagenes/Drone_Sensor_arq.png}
